...
export const createCommand = new Command()
	...
	.option(
		"--disk-size <diskSize>",
		`Disk size in GB, default is ${DEFAULT_DISK_SIZE}`,
	)
	.option(
		"--teepod-id <teepodId>",
		"TEEPod ID to use. If not provided, it will be selected from the list of available TEEPods.",
	)
	.option(
		"--image <image>",
		"Version of dstack image to use. If not provided, it will be selected from the list of available images for the selected TEEPod.",
	)
	...
	.action(async (options) => {
		try {
			...
			if (!options.teepodId) {
				...
				selectedTeepod = teepods.nodes.find(
					(pod) => pod.teepod_id === Number(options.teepodId),
				);
				if (!selectedTeepod) {
			...
			if (!options.image) {
				selectedImage = selectedTeepod.images?.find(
				...
				selectedImage = selectedTeepod.images?.find(
					(image) => image.name === options.image,
				);
				if (!selectedImage) {
