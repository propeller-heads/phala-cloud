		...
		for (const line of envFileContent.split("\n")) {
			...
			for (let i = 0; i < line.length; i++) {
				if (
					...
					if (delimiterIndex === -1) {
						// No '=' encountered yet, so this is a full-line comment essentially or malformed
			...
			for (let i = 0; i < value.length; i++) {
				if (
					...
					break;
				} else if (value[i] === "#" && !valueInQuotes && i === 0) {
					// Value starts directly with #
					commentStartIndex = i;
					break;
				}
			}
			...
			if (value === undefined) {
				// Should not happen if split correctly
				value = "";
			}
