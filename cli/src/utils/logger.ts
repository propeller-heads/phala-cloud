	...
	for (const word of words) {
		...
		if (
			currentLineVisibleLength +
				wordVisibleLength +
				(currentLineVisibleLength > 0 ? 1 : 0) >
			maxWidth
		) {
			lines.push(currentLine);
			currentLine = word;
...
export const logger = {
	...
	keyValueTable: <T extends object>(
		...
		const tableData = keys.map((key) => {
			...
			return {
				key:
					formatKeys && keyFormatter
						? chalk.cyan.bold(keyFormatter(key))
						: chalk.cyan.bold(key),
		...
		tableData.forEach((row, index) => {
			const keyLines = enableTextWrapping
				? wrapText(row.key, finalKeyWidth)
				: [row.key];
			const valueLines = enableTextWrapping
				? wrapText(row.value, finalValueWidth)
				: [row.value];
			...
			for (let i = 0; i < maxLines; i++) {
				...
				const keyContent =
					keyLine +
					" ".repeat(Math.max(0, finalKeyWidth - getVisibleLength(keyLine)));
				const valueContent =
					valueLine +
					" ".repeat(
						Math.max(0, finalValueWidth - getVisibleLength(valueLine)),
					);

				console.log(
					`${border.vertical} ${keyContent} ${border.vertical} ${valueContent} ${border.vertical}`,
				);
			}
			...
			if (index < tableData.length - 1) {
				console.log(
					`${border.leftT}${border.horizontal.repeat(finalKeyWidth + 2)}${border.cross}${border.horizontal.repeat(finalValueWidth + 2)}${border.rightT}`,
				);
			}
		});
...
function formatNestedObject(
	...
	for (const [key, value] of Object.entries(obj)) {
		if (value === null || value === undefined) {
			...
			lines.push(
				`${key}: ${formatNestedObject(value, currentDepth + 1, maxDepth)}`,
			);
		} else {
...
export function calculateColumnWidths<T>(
	...
	const totalBorderWidth =
		additionalBorderWidth + columns.length * borderCharsPerColumn;
		...
		for (const column of columns) {
			...
			if (column.fixedWidth === undefined && column.weight) {
				const extraWidth = Math.floor(
					remainingWidth * (column.weight / totalWeight),
				);
	...
	const calculatedTotalWidth =
		Object.values(initialWidths).reduce((sum, width) => sum + width, 0) +
		totalBorderWidth;

	if (calculatedTotalWidth > terminalWidth) {
		const scale =
			availableContentWidth / (calculatedTotalWidth - totalBorderWidth);

		const finalWidths: { [key: string]: number } = {};
		for (const column of columns) {
			const key = column.key as string;
			const minWidth = column.minWidth ?? column.header.length;
			finalWidths[key] = Math.max(
				minWidth,
				Math.floor(initialWidths[key] * scale),
			);
		}
...
export function formatTable<T>(
	...
	if (options.keyValueMode && data.length === 1) {
		...
		if (!options.columns) {
			options.columns = Object.keys(obj).map((key) => ({
				key,
				header:
					key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, " $1"), // Capitalize and add spaces
			}));
		}
	...
	if (!columns) {
		...
		columns = columns.map((col) => ({
			...col,
			enableTextWrapping:
				col.enableTextWrapping !== undefined
					? col.enableTextWrapping
					: mergedOptions.enableTextWrapping,
		}));
	}
	...
	const formatRow = (row: T, rowIndex: number) => {
		...
		const rowData = formattedColumns.map((col) => {
			...
			// Format the value
			let displayValue = col.formatter
				? col.formatter(value)
				: String(value || "");
	...
	if (mergedOptions.border) {
		...
		const topBorder =
			border.topLeft +
			formattedColumns
				.map((col) => border.horizontal.repeat(col.width + 2))
				.join(border.topT) +
			border.topRight;
		console.log(topBorder);
		...
		if (mergedOptions.includeHeaders) {
			...
			const wrappedHeaders = formattedColumns.map((col, i) => ({
				lines: col.enableTextWrapping
					? wrapText(headers[i], col.width)
					: [headers[i]],
				width: col.width,
			}));
			...
			const maxHeaderLines = Math.max(
				...wrappedHeaders.map((h) => h.lines.length),
			);
				...
				const headerLine =
					...
					wrappedHeaders
						.map((header) => {
							...
							return (
								" " +
								mergedOptions.headerStyle(content.padEnd(header.width)) +
								" "
							);
						})
		...
		if (mergedOptions.includeHeaders) {
			...
			// Display each line of the header
			for (let lineIndex = 0; lineIndex < maxHeaderLines; lineIndex++) {
				const headerLine = wrappedHeaders
					.map((header) => {
						const content = header.lines[lineIndex] || "";
			...
			const headerSeparator = formattedColumns
