...
export class DockerService {
	...
	private getLogFilePath(operation: string, image?: string): string {
		...
		return path.resolve(
			LOGS_DIR,
			`${image || this.image}-${operation}-${timestamp}.log`,
		);
	}
	...
	async buildImage(dockerfile: string, tag: string): Promise<boolean> {
		try {
			...
			const spinner = logger.startSpinner(
				`Building Docker image ${this.username}/${this.image}:${tag}`,
			);
	...
	async pushImage(imageName: string): Promise<boolean> {
		try {
			const spinner = logger.startSpinner(
				`Pushing Docker image ${imageName} to Docker Hub`,
			);
			...
			if (!credentials) {
				spinner.stop(false);
				throw new Error(
					'Docker credentials not found. Please log in first with "phala docker login"',
				);
			}
		...
		try {
			...
			try {
				...
				if (loginError.timedOut) {
					spinner.stop(false);
					throw new Error(
						"Docker login timed out. Please check your credentials and try again.",
					);
				}
				throw loginError;
			}
	...
	async checkLogin(): Promise<boolean> {
		try {
			...
			return !!(
				dockerConfig?.auths && Object.keys(dockerConfig.auths).length > 0
			);
		} catch (error) {
			logger.debug(
				`Docker login check failed: ${error instanceof Error ? error.message : String(error)}`,
			);
			return false;
		}
	}
	...
	async buildComposeFile(
		...
		if (envFile) {
			...
			envVars = envContent
				...
				.filter(
					Boolean as unknown as (value: string | null) => value is string,
				); // Remove null entries
		}
		...
		const compiledTemplate = Handlebars.compile(validatedTemplate.template, {
			noEscape: true,
		});
	...
	async runComposeLocally(
		...
		try {
			...
			const spinner = logger.startSpinner(
				`Running Docker Compose file at ${composePath}`,
			);
			...
			const composeArgs = ["-f", composePath, "up", "-d"];
			...
			logger.error(
				`Failed to run Docker Compose file: ${error instanceof Error ? error.message : String(error)}`,
			);
			return false;
		}
	}
	...
	async runSimulator(image: string, port: string): Promise<boolean> {
		try {
			...
			const { stdout } = await execAsync(
				`docker run -d --name tee-simulator --rm -p ${port}:${port} ${image}`,
			);
			const containerId = stdout.trim();

			logger.success(
				`TEE simulator running successfully. Container ID: ${containerId}`,
			);
			logger.break();
			logger.break();
			...
			logger.error(
				`Failed to run TEE simulator: ${error instanceof Error ? error.message : String(error)}`,
			);
			return false;
		}
	}
	...
	static async listLocalImages(): Promise<Array<{ imageName: string }>> {
		try {
			...
			logger.error(
				`Failed to list local Docker images: ${error instanceof Error ? error.message : String(error)}`,
			);
			return [];
		}
	}
