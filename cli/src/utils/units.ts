 ...
 */
export function parseSizeWithUnit(
	input: string,
	defaultUnit: "B" | "KB" | "MB" | "GB" | "TB" = "B",
): number {
	...
	if (!match) {
		throw new Error(
			`Invalid format: ${input}. Expected format: <number>[unit] (e.g., 2G, 500MB, 1T)`,
		);
	}
	...
	if (!(unit in units)) {
		throw new Error(
			`Unsupported unit: ${unit}. Supported units: B, K/KB, M/MB, G/GB, T/TB`,
		);
	}
...
export function parseMemoryInput(input: string): number {
	...
	if (memoryMB % 1024 !== 0) {
		throw new Error(
			`Memory must be a multiple of 1GB (1024MB). Got: ${memoryMB}MB`,
		);
	}
