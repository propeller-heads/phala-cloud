...
const createTempFile = (content: string): string => {
	...
	const tempFilePath = path.join(
		tempDir,
		`test-env-${Date.now()}-${Math.random().toString(36).substring(2, 7)}.env`,
	);
	...
	test("backticks are supported and maintain internal quotes", () => {
		...
		try {
			...
			expect(result).toEqual([
				{
					key: "BACKTICK_KEY",
					value: "This has 'single' and \"double\" quotes inside of it.",
				},
			]);
		} finally {
			cleanupTempFile(tempFile);
	...
	test("should handle various complex cases", () => {
		...
		try {
			...
			expect(result).toEqual([
				...
				{
					key: "SINGLE_QUOTED",
					value: "  single quoted value with spaces and # hash  ",
				},
				{
					key: "DOUBLE_QUOTED",
					value:
						"  double quoted value with spaces and # hash and newline \n next line ",
				},
				{
					key: "BACKTICK_QUOTED",
					value:
						"  backtick quoted value with spaces and # hash and newline \\\\\\\\n not expanded ",
				},
