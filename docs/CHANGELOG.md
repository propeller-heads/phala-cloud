...
### 2026-01-21 (Latest)
- **New CVM response structure**: Introduces `CVMInfo` / `CVMInfoDetail` types with DB-only queries for significantly faster responses
- **New Auth response structure**: User/auth endpoints now include workspace context
- **Apps endpoints**: Use full `CVMInfo` for app CVM listings
- **New fields**: `created_at`, `deleted_at`, `listed`, `project_type`, and more added to CVM responses
...
- To use a specific version, explicitly set the version:
  ```typescript
  const client = createClient({
    apiKey: 'your-api-key',
    version: '2025-10-28'  // Pin to a specific version
  })
  ```
